...
int main(int argc, char**argv) {
  ...
  /* constant section for O-DIRECT */
#ifdef O_DIRECT
  fprintf(output, "(cl:defconstant ");
  fprintf(output, "o-direct");
  fprintf(output, " ");
  if(SIGNED64P(O_DIRECT))
    fprintf(output, "%lli", (int64_t) O_DIRECT);
  else
    fprintf(output, "%llu", (uint64_t) O_DIRECT);
  fprintf(output, ")\n");
#else
  fprintf(output, "(cl:warn 'cffi-grovel:missing-definition :name 'O-DIRECT)\n");
#endif

  /* constant section for O-DIRECTORY */
#ifdef O_DIRECTORY
  fprintf(output, "(cl:defconstant ");
  fprintf(output, "o-directory");
  fprintf(output, " ");
  if(SIGNED64P(O_DIRECTORY))
    fprintf(output, "%lli", (int64_t) O_DIRECTORY);
  else
    fprintf(output, "%llu", (uint64_t) O_DIRECTORY);
  fprintf(output, ")\n");
#else
  fprintf(output, "(cl:warn 'cffi-grovel:missing-definition :name 'O-DIRECTORY)\n");
#endif

  /* constant section for O-LARGEFILE */
#ifdef O_LARGEFILE
  fprintf(output, "(cl:defconstant ");
  fprintf(output, "o-largefile");
  fprintf(output, " ");
  if(SIGNED64P(O_LARGEFILE))
    fprintf(output, "%lli", (int64_t) O_LARGEFILE);
  else
    fprintf(output, "%llu", (uint64_t) O_LARGEFILE);
  fprintf(output, ")\n");
#else
  fprintf(output, "(cl:warn 'cffi-grovel:missing-definition :name 'O-LARGEFILE)\n");
#endif

  /* constant section for O-DSYNC */
#ifdef O_DSYNC
  fprintf(output, "(cl:defconstant ");
  fprintf(output, "o-dsync");
  fprintf(output, " ");
  if(SIGNED64P(O_DSYNC))
    fprintf(output, "%lli", (int64_t) O_DSYNC);
  else
    fprintf(output, "%llu", (uint64_t) O_DSYNC);
  fprintf(output, ")\n");
#else
  fprintf(output, "(cl:warn 'cffi-grovel:missing-definition :name 'O-DSYNC)\n");
#endif

  /* constant section for O-RSYNC */
#ifdef O_RSYNC
  fprintf(output, "(cl:defconstant ");
  fprintf(output, "o-rsync");
  fprintf(output, " ");
  if(SIGNED64P(O_RSYNC))
    fprintf(output, "%lli", (int64_t) O_RSYNC);
  else
    fprintf(output, "%llu", (uint64_t) O_RSYNC);
  fprintf(output, ")\n");
#else
  fprintf(output, "(cl:warn 'cffi-grovel:missing-definition :name 'O-RSYNC)\n");
#endif
